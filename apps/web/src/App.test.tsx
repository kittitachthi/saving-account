...
describe("แดชบอร์ดการเงินประจำวัน", () => {
  ...
  it("เพิ่มรายจ่าย อัปเดตยอด และบันทึกลง localStorage", async () => {
    ...
    await user.click(
      within(form).getByRole("button", { name: "บันทึกรายการ" }),
    );
    expect(screen.getByText("ค่าขนม")).toBeInTheDocument();
    expect(screen.getByText("฿30,890", { exact: false })).toBeInTheDocument();
    expect(localStorage.getItem("daily-money-transactions-v2")).toContain(
      "ค่าขนม",
    );
  });
  ...
  it("โหลดเฉพาะรายการที่ถูกต้องจาก localStorage", () => {
    ...
    expect(
      screen.getAllByText("฿800", { exact: false }).length,
    ).toBeGreaterThan(0);
  });
  ...
  it("เพิ่มเงินเก็บ แยกหมวด และลดยอดเงินพร้อมใช้", async () => {
    ...
    await user.selectOptions(
      within(form).getByLabelText("ประเภทเงินเก็บ"),
      "ท่องเที่ยว",
    );
    await user.click(
      within(form).getByRole("button", { name: "บันทึกรายการ" }),
    ...
    expect(
      screen.getAllByText("฿5,000", { exact: false }).length,
    ).toBeGreaterThan(0);
  });
  ...
  it("ปฏิเสธรายจ่ายที่มากกว่าเงินพร้อมใช้และแจ้งส่วนที่เกิน", async () => {
    ...
    expect(within(form).getByRole("alert")).toHaveTextContent(
      "ยอดรายจ่ายมากกว่าเงินพร้อมใช้ 9,010 บาท",
    );
  ...
  it("ลบรายการและปรับยอดรวมกับ localStorage ให้สอดคล้องกัน", async () => {
    ...
    expect(localStorage.getItem("daily-money-transactions-v2")).not.toContain(
      "กาแฟ",
    );
  });
  ...
  it("แสดงรายละเอียดหมวดเงินเก็บเมื่อ focus ส่วนกราฟ", async () => {
    ...
    const chart = screen.getByRole("group", {
      name: "กราฟความคืบหน้าเงินเก็บ",
    });
    within(chart)
      .getByRole("button", { name: "เงินฉุกเฉิน 80% ของเงินเก็บ" })
      .focus();
    const tooltip = await screen.findByRole("tooltip");
  ...
  it("ชั้นข้อความกลางกราฟไม่ดัก pointer และ hover ส่วนสีเปิด tooltip", async () => {
    ...
    const user = userEvent.setup();
    render(<App />);
    expect(screen.getByTestId("savings-chart-center")).toHaveStyle({
      pointerEvents: "none",
    });
    const chart = screen.getByRole("group", {
      name: "กราฟความคืบหน้าเงินเก็บ",
    ...
    await user.hover(
      within(chart).getByRole("button", {
        name: "เงินฉุกเฉิน 100% ของเงินเก็บ",
      }),
    );
  ...
  it("Floating Tooltip ตาม pointer และหยุดตำแหน่งเมื่อ pin", async () => {
    ...
    const segment = within(chart).getByRole("button", {
      name: "ท่องเที่ยว 100% ของเงินเก็บ",
    });
  ...
  it("Tooltip ที่ pin ไม่เปลี่ยนจาก hover และสลับเมื่อเลือกหมวดใหม่", async () => {
    ...
    const emergency = within(chart).getByRole("button", {
      name: "เงินฉุกเฉิน 67% ของเงินเก็บ",
    });
    const travel = within(chart).getByRole("button", {
      name: "ท่องเที่ยว 33% ของเงินเก็บ",
    });
  ...
  it("ใช้ Theme Preference เหนือ System Theme และเปลี่ยนผ่านการตั้งค่า", async () => {
    ...
    const toggle = within(settings).getByRole("button", {
      name: "เปลี่ยนเป็นธีมสว่าง",
    });
  ...
  it("เริ่มด้วยรายการว่างและไม่แสดง Pagination", () => {
    ...
    expect(
      screen.queryByRole("navigation", { name: "หน้ารายการธุรกรรม" }),
  ...
  it("แบ่งหน้าหลังกรองและกลับหน้าแรกเมื่อเปลี่ยนตัวกรอง", async () => {
    ...
    await user.click(within(filters).getByRole("button", { name: "รายรับ" }));
    expect(
      within(pagination).getByRole("button", { name: "1" }),
    ).toHaveAttribute("aria-current", "page");
  });
