...
const elementAnchor = (element: Element): Point =>
  calculateTriggerAnchor(element.getBoundingClientRect());
...
export function useFloatingTooltip<T>(
  items: T[],
  renderTooltip: (item: T, pinned: boolean) => ReactNode,
) {
  ...
  const hover = (index: number, point: Point) => {
    if (!pinned) {
  ...
  const pointerEnter = (index: number) => (event: PointerEvent<Element>) =>
    hover(index, pointerPoint(event));
  const pointerMove = (index: number) => (event: PointerEvent<Element>) =>
    hover(index, pointerPoint(event));
  const pointerDown = (event: PointerEvent<Element>) => {
    lastPointerType.current = event.pointerType;
  };
  const focus = (index: number) => (event: FocusEvent<Element>) =>
    hover(index, elementAnchor(event.currentTarget));
  const click = (index: number) => (event: React.MouseEvent<Element>) =>
    togglePin(
      index,
      lastPointerType.current === "touch" || event.detail === 0
        ? elementAnchor(event.currentTarget)
        : pointerPoint(event),
    );
  const keyDown =
    (index: number, manualActivation = false) =>
    (event: KeyboardEvent<Element>) => {
      if (manualActivation && (event.key === "Enter" || event.key === " ")) {
        event.preventDefault();
        togglePin(index, elementAnchor(event.currentTarget));
      }
  ...
  const tooltip =
    activeIndex === null
      ...
      : createPortal(
          <div
            ref={tooltipRef}
            className={`${styles.floatingTooltip} ${pinned ? styles.pinnedTooltip : ""}`}
            role="tooltip"
