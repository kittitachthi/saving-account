...
export const loadTransactions=(_fallback:Transaction[]=[],storage:Storage=localStorage):Transaction[]=>{try{storage.removeItem(LEGACY_TRANSACTIONS_KEY);const saved=storage.getItem(TRANSACTIONS_KEY);if(!saved)return [];const parsed:unknown=JSON.parse(saved);return Array.isArray(parsed)?parsed.filter(isTransaction):[]}catch{return []}}
